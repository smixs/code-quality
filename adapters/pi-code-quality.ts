...
// The skill works from any install folder: the script is the one next to this adapter, unless
// QG_SCRIPT names another.
export const scriptPath = (env: NodeJS.ProcessEnv = process.env) => (env.QG_SCRIPT ?? "").trim() || resolve(import.meta.dir, "../scripts/quality.ts");
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    ...
    const r = await pi.exec("sh", ["-c", 'printf "%s" "$1" | exec bun "$2" agent-stop', "sh", input, scriptPath()], { timeout: 600_000 });
